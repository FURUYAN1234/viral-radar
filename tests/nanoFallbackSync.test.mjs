...
test('nano banana pro fallback snapshot is synced from source model arrays', async () => {
  ...
  assert.equal(NANO_BANANA_FALLBACK_SNAPSHOT.chains.openaiText.priceSnapshotDate, '2026-09-23');
  assert.equal(NANO_BANANA_FALLBACK_SNAPSHOT.chains.openaiText.models[0].label, 'GPT-6 Astra');
  assert.equal(NANO_BANANA_FALLBACK_SNAPSHOT.chains.openaiText.models[4].inputPriceUsdPerM, 0.1);
