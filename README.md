...
公開Web/RSSで実際に取得した話題を、漫画、ショート動画、トレンド解説動画、小説の「そのまま制作に入れる企画」へ変換する創作支援アプリです。

単なる流行語リストではなく、取得根拠、読者・視聴者の欲求、媒体別の勝ち筋、物語設計、本文生成プロンプト、編集者向け企画書までを一画面で扱います。
...
物語バズメーカーは、ニュースやSNS周辺で反応が起きている話題を、創作者が扱える「感情」「構造」「冒頭」「主人公」「葛藤」「転換」「読者維持」に変換します。

流行語をそのまま作品に入れるのではなく、以下の順序で企画へ落とします。

1. 公開Web/RSSから話題候補を取得する。
2. 取得元、リンク、検索クエリ、観測時刻、算出指標を残す。
3. 話題の表面語ではなく、読者・視聴者が反応する欲求に分解する。
4. 漫画、ショート動画、解説動画、小説それぞれの表現単位に変換する。
5. プロット、画面構成、台本、章構成、本文生成プロンプトへ落とす。
6. 必要に応じてOpenAIまたはGeminiで詳細分析と参考本文を生成する。
...
本アプリでは、同じ根拠を見ても、媒体ごとに別の制作判断へ変換します。
...
| 分析 | スコア、チャート、根拠カード | 何を見るべきかを作品判断に変える |
| 企画 | そのまま企画に使える案 | タイトル、主人公、葛藤、転換を出す |
| 物語設計 | 伏線と回収、GMC+S、感情差分 | 面白さの骨組みを明示する |
| 媒体設計 | 漫画、動画、小説別の制作メモ | 実際の制作手順へ落とす |
...
企画案では、タイトル候補だけでなく、主人公、舞台、最初の事件、対立、最後の選択、読後感まで出します。
...
短編・中編・長編の読者維持設計まで落とし込むWeb小説企画を作ります。
...
小説では、本文生成プロンプトにも短編・中編・長編別の読者維持設計を含めます。
...
読み取り欄は、同じ定型文を並べるのではなく、取得した各話題の内容と媒体に合わせて変換します。

### Decision Scores / 制作判断チャート

RSSや公開フィードから取れる情報をもとに、制作判断用のスコアを出します。
...
- 根拠量: 使用できる観測が十分あるか。
- 企画化しやすさ: 物語や台本へ落とせる感情・状況があるか。

これらは装飾用の架空数字ではなく、取得結果からの計算値です。ただし、TikTok再生数やYouTube視聴回数のような未取得の実数は表示しません。

### Creator Roadmap / 制作ロードマップ

全くの素人でも最初の制作に入れるよう、カテゴリごとに制作順を出します。
...
タイトルは機能説明ではなく、作品として成立する候補を出す方針です。

### Story Architecture / 物語・台本設計

企画案には、単なる説明ではなく、内部の物語設計を表示します。

含まれる観点:

- 伏線と回収。
- GMC+S: Goal, Motivation, Conflict, Stakes。
- 感情差分。
- モチーフ再登場。
- 知識境界。
- 媒体実装。

これにより、読者が読み進める理由を、説明文ではなく設計として確認できます。

### AI Deep Analysis / API詳細分析

右上の `API設定` にOpenAIまたはGeminiのキーを入力すると、取得結果をもとに詳細分析を実行します。

画面ではプロバイダ名をタイトルに混ぜず、分析内容そのものを表示します。
...
- タイトルから始まる。
- 本文、台本、場面描写の形になっている。
- 空白、アウトラインだけ、Markdown見出しだけを成功扱いにしない。
- 生成に失敗した場合は理由を表示する。
...
### Not Implemented / 未実装の取得

以下は現時点で直接の公式API取得をしていません。

- TikTok Creative Center API。
- YouTube Data APIの実視聴回数取得。
- Google Trends非公式スクレイピング。
- SNSコメント数の直接クロール。

未実装のデータを、取得済みのように見せることは禁止しています。
...
### API Key Handling / APIキーの扱い

- APIキーはチャットに貼らないでください。
- APIキーはアプリ画面の右上 `API設定` から入力します。
- 保存済みキー全文は入力欄に再表示しません。
- JSON、Markdown、DOCX、HANDOFF、PLAN、ログへキーを含めません。
- 共有PCでは保存機能を使わないでください。
- キーを変更する場合は `API設定` から新しいキーを保存します。

### Local Provider Proxy / ローカル中継

OpenAIのブラウザ直接呼び出しはCORS制限を受けるため、ローカルViteの `/api/provider-generate` を通してAPIへ送ります。

この中継はローカル開発サーバー内の処理です。キーをファイルへ書き出しません。

### Fallback Policy / フォールバック方針

モデルの試行順は、保守時に既存アプリの安定順を確認してスナップショット化します。

実行時に他アプリを呼び出すわけではありません。
...
差分が出た場合は、変更内容を確認してから同期します。

```powershell
npm run sync:story-maker
npm run sync:nano-fallback
```

この仕組みは、創作ロジックやAPIモデル順の劣化を避けるためのメンテナンス手順です。アプリ本体は独立して動作します。
...
- `npm test` が失敗していない。
- `npm run build` が失敗していない。
- ブラウザで `http://127.0.0.1:5180/` を開き、UIが崩れていない。
- 公開Web/RSS取得が実際に走る。
- カテゴリ選択で同じテンプレートだけが出ていない。
- API詳細分析中にUIが非活性化される。
- 参考本文生成が空白やアウトラインだけを成功扱いにしていない。
- JSON/DOCXにAPIキーが入っていない。
- DOCXがWordで開ける。
...
企画案は、固定サンプルではなく、取得した観測結果をもとに作ります。

取得結果のタイトルや固有名詞をそのまま作品名にするのではなく、感情タイプ、葛藤、主人公の入口、架空化の方針へ変換します。

### 2. Medium Translation Layer / 媒体変換レイヤー

同じ話題を、漫画、ショート動画、解説動画、小説で別の制作単位へ変換します。
...
DOCXはWeb画面の複製ではなく、打ち合わせで読める企画書へ再構成します。

プロンプト全文やJSON丸写しは、編集者に見せる文書として不要なため含めません。
...
本編や企画案では、実在名を避け、架空制度、架空UI、架空組織、架空の町、架空の生活場面へ置き換えます。
...
├── start_monogatari_buzz_maker.bat
├── scripts/
│   ├── sync-nano-fallback-chain.mjs
│   └── sync-story-maker-routines.mjs
├── src/
│   ├── main.js
│   ├── styles.css
│   └── lib/
...
│       ├── exporters.js
│       ├── fileNames.js
│       ├── nanoBananaFallbackSnapshot.js
│       ├── providers.js
│       ├── reportEngine.js
│       ├── scoring.js
│       ├── settings.js
│       └── storyMakerRoutineSnapshot.js
└── tests/
    ├── browser-smoke.mjs
    ├── docxExporter.test.mjs
    ├── exporters.test.mjs
    ├── providers.test.mjs
    ├── reportEngine.test.mjs
    ├── storyMakerRoutineSync.test.mjs
    ├── trendSearch.test.mjs
    └── trendSearchLogic.test.mjs
```
...
`npm run deploy` は `npm run build` を先に実行し、`dist/` を `gh-pages` ブランチへ公開します。

デプロイ前の必須確認:

1. バージョンが上がっている。
2. `npm test` が通っている。
3. `npm run build` が通っている。
4. ブラウザ実機確認が終わっている。
5. README、JSON、DOCX、ログにAPIキーがない。
6. 不要な生成物、個人情報、ローカルパスがコミット対象に入っていない。

バックアップは、ユーザーが明示的に依頼した場合のみ実行します。

---

## Known Limitations / 既知の制限

- TikTok、YouTube、SNS各社の内部指標は直接取得していません。
- Google Trendsの非公開データや詳細指数は取得していません。
- 現在の取得は公開Web/RSS/公開フィード範囲です。
- RSS取得元の停止、仕様変更、アクセス制限により結果が減ることがあります。
- API詳細分析と参考本文生成は、有効なOpenAIまたはGeminiキーに依存します。
- 出力は創作補助であり、ヒットや出版を保証しません。
- 根拠欄で実在名を表示する場合がありますが、創作本編では架空化する前提です。
...
- 深掘り分析にも分析ラウンドと中心根拠を反映し、カテゴリ固定文だけにならない回帰テストを追加。
...
- DOCX企画書が編集者との打ち合わせ用文書として開けること、APIキーやプロンプトを含まないことを確認。
...
| Repository | `viral-radar` |
| Local Port | `5180` |
| Public URL | `https://FURUYAN1234.github.io/viral-radar/` |
| License | `UNLICENSED` |
